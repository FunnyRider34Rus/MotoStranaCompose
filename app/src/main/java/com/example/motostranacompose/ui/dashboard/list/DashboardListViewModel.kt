...
@HiltViewModel
class DashboardListViewModel @Inject constructor(private val useCases: DashboardUseCases) : ViewModel(), EventHandler<DashboardListEvent> {
        ...
        useCases.getAll().collect { response ->
            when (response) {
                ...
                is Success -> {
                    ...
                    _viewState.value = _viewState.value.copy(content = response.data ?: emptyList())
                }
                is Failure -> {
                    _viewState.value = _viewState.value.copy(error = response.e?.message ?: "An unexpected error occurred")
                }
            }
        }
    }
